...
/*Wrapper for RL's NDEF_prep function, preapares an array large enough for the whole
 *message based upon the size of the incoming payload */
void nfc_send(byte message[]){
  int PAY_LEN; 
  PAY_LEN=sizeof(message);                    //find the length of the payload
  byte NDEF_MSG[PAY_LEN + PRE_PAY_LEN-1];     //create the full length array 
  int NDEF_LEN = sizeof(NDEF_MSG);            //store its length in an int
  NDEF_prep(NDEF_MSG, PAY_LEN, message);      //prepare the full length message
}


/*Comparison function which checks if 2 arrays are equal to each other*/
